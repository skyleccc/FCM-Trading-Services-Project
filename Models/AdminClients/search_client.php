...
if (isset($_POST['query'])) {
    ...
    $clientListQuery = "
        SELECT c.*, p.projectID, p.completionDate AS latestProjectDate
        FROM client c
        LEFT JOIN (
    ...
    ";
    $stmt = $conn->prepare($clientListQuery);
    $likeQuery = "%" . $searchQuery . "%";
    $stmt->bind_param("s", $likeQuery);
    $stmt->execute();
    $clientListResult = $stmt->get_result();
} else {
    $clientListQuery = "
        SELECT c.*, p.projectID, p.completionDate AS latestProjectDate
        FROM client c
        ...
        echo'
            <div class="client box">
                ...
                <div class="action-buttons h100-w25">
                    <button type="button edit-btn" data-id="'.$clientListRow["clientID"].'" class="btn btn-primary" data-toggle="modal" data-target="#editClientModal">
