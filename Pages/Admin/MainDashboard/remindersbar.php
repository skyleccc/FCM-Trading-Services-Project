<?php
    ...
    $sql2 = "SELECT phase.phasetitle, phase.phasedescription, project.projectname, phase.phaseid, project.projectid, phase.expectedFinishDate FROM phase, project WHERE project.projectid = phase.projectid AND phase.isFinished != TRUE AND phase.expectedFinishDate = CURDATE();"; 
    ...
    if ($result2->num_rows > 0) {
        while ($row = $result2->fetch_assoc()) {
            echo '
            <a href="'. $redirectTo . htmlspecialchars($row["projectid"]) . '"  class="row p-1 border bg light" style="margin-bottom: 25px; text-decoration: none">
